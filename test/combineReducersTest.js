...
describe('combineReducers', () => {
  ...
  it('basic test', () => {
    ...
    expect(reducer(undefined, {type: 'a'})).to.equal(Map({a: 1, b: 0, c: 0}))
    expect(reducer(undefined, {type: 'ab'})).to.equal(Map({a: 1, b: 1, c: 0}))
    expect(reducer(undefined, {type: 'abc'})).to.equal(Map({a: 1, b: 1, c: 1}))
  })
  ...
  it('on mix of reducers with and without actionHandlers', () => {
    ...
    expect(reducer(undefined, {type: 'a'})).to.equal(Map({a: 1, b: 0, c: 0, d: 0}))
    expect(reducer(undefined, {type: 'ab'})).to.equal(Map({a: 1, b: 1, c: 0, d: 0}))
    expect(reducer(undefined, {type: 'abc'})).to.equal(Map({a: 1, b: 1, c: 1, d: 0}))
    expect(reducer(undefined, {type: 'abcd'})).to.equal(Map({a: 1, b: 1, c: 1, d: 1}))
    expect(reducer(Map({a: 5, b: 6, c: 7, d: 8}), {type: 'abcd'})).to.equal(Map({a: 6, b: 7, c: 8, d: 9}))
  })
  it('missing initial value edge case', () => {
    ...
    expect(reducer(Map({c: 0}), {type: 'ab'})).to.equal(Map({a: 1, b: 1, c: 0}))
  })
